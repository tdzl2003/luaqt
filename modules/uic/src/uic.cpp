...
void Uic::writeCopyrightHeader(DomUI *ui)
{
	...
	if (option().generator == Option::LuaGenerator){
		if (comment.size())
			out << "--[[\n" << comment << "\n]]\n\n";
...
#endif
    } else if (option().generator == Option::LuaGenerator) {
#ifdef QT_UIC_LUA_GENERATOR
        if (!language.isEmpty() && language.toLower() != QLatin1String("lua")) {
            fprintf(stderr, "uic: File is not a 'lua' ui file, language=%s\n", qPrintable(language));
            return false;
        }

        rtn = luawrite (ui);
#else
        fprintf(stderr, "uic: option to generate lua code not compiled in\n");
#endif
	} else {
#ifdef QT_UIC_CPP_GENERATOR
